...
static int client_net_recv( void *ctx, unsigned char *buf, size_t len ) {
  ...
  // if (result > 0) {
    //esp_log_buffer_hexdump_internal("SSL.RD", buf, (uint16_t)result, ESP_// log_vERBOSE);
  // }
...
static int client_net_send(void *ctx, const unsigned char *buf, size_t len) {
  ...
  // esp_log_buffer_hexdump_internal("SSL.WR", buf, (uint16_t)len, ESP_// log_vERBOSE);BEDTLS_ERR_NET_SEND_FAILED;
 ...
 */
int start_ssl_client(sslclient__context *ssl_client,  const char *host,  uint32_t port,  int timeout,  const char *rootCABuff,  bool useRootCABundle,  const char *cli_cert,
                     const char *cli_key,  const char *pskIdent,  const char *psKey,  bool insecure,  const char **alpn_protos)
{

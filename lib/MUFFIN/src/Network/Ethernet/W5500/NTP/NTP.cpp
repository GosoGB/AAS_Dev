...
//     constexpr uint8_t MAX_SNTP_BUF_SIZE = sizeof(ntp_fmt_t);  // maximum size of DNS buffer
//     constexpr uint8_t NTP_SERVER_PORT = 123;
//     constexpr uint32_t SECS_PERDAY = DAY_IN_SEC;
...
//     void NTP::fetchSeconds(uint16_t idx, uint8_t* buf)
//     {
//         uint64_t seconds = 0;
//         uint8_t i=0;
...
//     void NTP::Init(Socket& socket, IPAddress host, uint8_t tz, uint8_t* buf)
//     {
//         mSocket = &socket;

//         mNtpData.dstaddr[0] = host[0];
//         mNtpData.dstaddr[1] = host[1];
//         mNtpData.dstaddr[2] = host[2];
//         mNtpData.dstaddr[3] = host[3];

//         mTimezone = tz;
//         data_buf = buf;

//         mNtpData.leap = 0;           /* leap indicator */
//         mNtpData.version = 4;        /* version number */
//         mNtpData.mode = 3;           /* mode */
//         mNtpData.stratum = 0;        /* stratum */
//         mNtpData.poll = 0;           /* poll interval */
//         mNtpData.precision = 0;      /* precision */
//         mNtpData.rootdelay = 0;      /* root delay */
//         mNtpData.rootdisp = 0;       /* root dispersion */
//         mNtpData.refid = 0;          /* reference ID */
//         mNtpData.reftime = 0;        /* reference time */
//         mNtpData.org = 0;            /* origin timestamp */
//         mNtpData.rec = 0;            /* receive timestamp */
//         mNtpData.xmt = 1;            /* transmit timestamp */

//         uint8_t Flag;
//         Flag = (mNtpData.leap << 6) + (mNtpData.version << 3) + mNtpData.mode; //one byte Flag
//         memcpy(mNtpMessage, (void const*)(&Flag), 1);
//     }


//     Status NTP::Fetch(datetime_t* output)
//     {
//         uint16_t RSR_len;
//         uint32_t destip = 0;
//         uint16_t destport;
...
//         Status ret(Status::Code::BAD);

//         const ssr_e socketStatus = mSocket->GetStatus();
//         if (socketStatus != ssr_e::INIT_UDP)
//         {
//             LOG_ERROR(logger, "SOCKET MUST BE OPENED IN UDP MODE");
//             return ret;
//         }
...
//         const int receivedLength = mSocket->Available();
//         if (receivedLength == -1)
//         {
//             LOG_ERROR(logger, "FAILED TO RETRIEVE RECEIVED DATA SIZE");
//             return ret;
//         }

//         mSocket->ReceiveFrom()
        
//         if ((RSR_len = getSn_RX_RSR(mSocket)) > 0)
//         {
//             if (RSR_len > MAX_SNTP_BUF_SIZE)
//                 RSR_len = MAX_SNTP_BUF_SIZE;	// if Rx data size is lager than TX_RX_MAX_BUF_SIZE
            
//             recvfrom(mSocket, data_buf, RSR_len, (uint8_t *)&destip, &destport);

//             fetchSeconds(data_buf,startindex);
...
//                 {
//                     sendto(mSocket,mNtpMessage,sizeof(mNtpMessage),mNtpData.dstaddr,ntp_port);
//                     ntp_retry_cnt++;
//                 }
...
//                         sendto(mSocket,mNtpMessage,sizeof(mNtpMessage),mNtpData.dstaddr,ntp_port, 4);
//     #else
//                         sendto(mSocket,mNtpMessage,sizeof(mNtpMessage),mNtpData.dstaddr,ntp_port);
//     #endif
//     #else
...
//     void NTP::calculateDatetime(const uint64_t seconds)
//     {
//         uint8_t yf=0;
//         uint8_t leap;
//         uint64_t n=0,d=0,total_d=0,rz=0;
...
//     uint64_t NTP::changeDatetime2Seconds()
//     {
//         uint64_t seconds=0;

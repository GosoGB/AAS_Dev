...
bool readLogicalValue( EIPSession& session, uint8_t classId, uint8_t instanceId, uint8_t attributeId, uint16_t dataType, cip_data_t& outResult)
{
    ...
    muffin::psram::vector<uint8_t> path = buildLogicalPath(classId, instanceId, attributeId);
...
// 쓰기 함수 (데이터 타입 파라미터화)
bool writeLogicalValue(EIPSession& session, uint8_t classId, uint8_t instanceId, uint8_t attributeId, uint16_t dataType, const muffin::psram::vector<uint8_t>& data, cip_data_t& outResult)
{
    constexpr size_t CIP_OFFSET = 40;

 ...
 * 
 * @brief TCP Transport 클래스를 정의합니다.
 * 
 * @date 2025-04-07
 * @version 1.4.0
 * 
...
namespace muffin
{
    ...
    std::string TCPTransport::sendAndReceive(const std::string &command) 
    {
        if (!client.connected()) 
        {
        ...
        std::string response = "";
        int cmdLen = static_cast<int>(command.length());
        // log_d("DATA : %s \n\n ",command.c_str());

        client.write(reinterpret_cast<const uint8_t*>(command.c_str()), cmdLen);
        ...
        for (int i = 0; i < len; i++) 
        {
            response += static_cast<char>(resp[i]);
        }

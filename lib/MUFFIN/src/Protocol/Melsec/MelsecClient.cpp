...
namespace muffin
{
    ...
    bool MelsecClient::Begin(IPAddress ip, uint16_t port) 
    {
    ...
    bool MelsecClient::WriteWords(jvs::node_area_e area, uint32_t address, size_t count, const uint16_t value[])
    {
        ...
        if (mDataFormat == jvs::df_e::ASCII) 
        {
            ...
            idx = mMelsecBuilder.BuildWriteRequestDataASCII(mCommonHeader, false, area, address, count, value, mReqFrame);
        ...
        else 
        {
            ...
            idx = mMelsecBuilder.BuildWriteRequestDataBinary(mCommonHeader, false, area, address, count, value, mReqFrame);
            size_t respSize =  sendAndReceive(mReqFrame, idx, mRespFrame);
            if (respSize == 0)
            {
                LOG_ERROR(logger, "CONNECTION ERROR");
    ...
    bool MelsecClient::WriteBits(jvs::node_area_e area, uint32_t address, size_t count, const uint16_t value[])
    {
        ...
        if (mDataFormat == jvs::df_e::ASCII) 
        {
            ...
            idx = mMelsecBuilder.BuildWriteRequestDataASCII(mCommonHeader, true, area, address, count, value, mReqFrame);
            size_t respSize =  sendAndReceive(mReqFrame, idx, mRespFrame);
            if (respSize == 0)
            {
                LOG_ERROR(logger, "CONNECTION ERROR");
        ...
        else 
        {
            ...
            idx = mMelsecBuilder.BuildWriteRequestDataBinary(mCommonHeader, true, area, address, count, value, mReqFrame);
            size_t respSize =  sendAndReceive(mReqFrame, idx, mRespFrame);
            if (respSize == 0)
            {
                LOG_ERROR(logger, "CONNECTION ERROR");
    ...
    int MelsecClient::ReadWords(jvs::node_area_e area, uint32_t address, size_t count, uint16_t buffer[]) 
    {
        ...
        if (mDataFormat == jvs::df_e::ASCII) 
        {
            ...
            idx = mMelsecBuilder.BuildReadRequestDataASCII(mCommonHeader, false, area, address, count, mReqFrame);
            size_t respSize =  sendAndReceive(mReqFrame, idx, mRespFrame);
            if (respSize == 0)
            {
                LOG_ERROR(logger, "CONNECTION ERROR");
        ...
        else
        {
            ...
            idx = mMelsecBuilder.BuildReadRequestDataBinary(mCommonHeader, false, area, address, count, mReqFrame);
            size_t respSize =  sendAndReceive(mReqFrame, idx, mRespFrame);
            if (respSize == 0)
            {
                LOG_ERROR(logger, "CONNECTION ERROR");
    ...
    int MelsecClient::ReadBits(jvs::node_area_e area, uint32_t address, size_t count, uint16_t buffer[]) 
    {
        ...
        if (mDataFormat == jvs::df_e::ASCII) 
        {
            ...
            idx = mMelsecBuilder.BuildReadRequestDataASCII(mCommonHeader, true, area, address, count, mReqFrame);
            size_t respSize =  sendAndReceive(mReqFrame, idx, mRespFrame);
            if (respSize == 0)
            {
                LOG_ERROR(logger, "CONNECTION ERROR");
        ...
        else
        {
            ...
            idx = mMelsecBuilder.BuildReadRequestDataBinary(mCommonHeader, true, area, address, count, mReqFrame);
            size_t respSize = sendAndReceive(mReqFrame, idx, mRespFrame);
            if (respSize == 0)
            {
                LOG_ERROR(logger, "CONNECTION ERROR");
